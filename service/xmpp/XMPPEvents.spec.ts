...
describe("/service/xmpp/XMPPEvents members", () => {
    const { XMPPEvents, ...others } = exported;

    it("known members", () => {
        expect(XMPPEvents).toBeDefined();

        expect(XMPPEvents.ADD_ICE_CANDIDATE_FAILED).toBe(
            "xmpp.add_ice_candidate_failed",
        );
        expect(XMPPEvents.AUDIO_MUTED_BY_FOCUS).toBe(
            "xmpp.audio_muted_by_focus",
        );
        expect(XMPPEvents.VIDEO_MUTED_BY_FOCUS).toBe(
            "xmpp.video_muted_by_focus",
        );
        expect(XMPPEvents.AUTHENTICATION_REQUIRED).toBe(
        ...
        expect(XMPPEvents.SETTINGS_ERROR_RECEIVED).toBe(
            "xmpp.settings_error_received",
        );
        expect(XMPPEvents.CONFERENCE_PROPERTIES_CHANGED).toBe(
            "xmpp.conference_properties_changed",
        );
        expect(XMPPEvents.CONNECTION_ESTABLISHED).toBe(
            "xmpp.connection.connected",
        );
        expect(XMPPEvents.CONNECTION_FAILED).toBe("xmpp.connection.failed");
        expect(XMPPEvents.CONNECTION_INTERRUPTED).toBe(
            "xmpp.connection.interrupted",
        );
        expect(XMPPEvents.CONNECTION_RESTORED).toBe("xmpp.connection.restored");
        expect(XMPPEvents.CONNECTION_ICE_FAILED).toBe(
            "xmpp.connection.ice.failed",
        );
        expect(XMPPEvents.CONNECTION_RESTARTED).toBe("xmpp.connection.restart");
        expect(XMPPEvents.CONNECTION_STATUS_CHANGED).toBe(
            "xmpp.connection.status.changed",
        );
        expect(XMPPEvents.DISPLAY_NAME_CHANGED).toBe(
        ...
        expect(XMPPEvents.INVITE_MESSAGE_RECEIVED).toBe(
            "xmpp.invite_message_received",
        );
        expect(XMPPEvents.PRIVATE_MESSAGE_RECEIVED).toBe(
            "xmpp.private_message_received",
        );
        expect(XMPPEvents.MUC_MEMBER_BOT_TYPE_CHANGED).toBe(
            "xmpp.muc_member_bot_type_changed",
        );
        expect(XMPPEvents.MUC_DESTROYED).toBe("xmpp.muc_destroyed");
        expect(XMPPEvents.MUC_JOIN_IN_PROGRESS).toBe(
            "xmpp.muc_join_in_progress",
        );
        ...
        expect(XMPPEvents.MUC_LOBBY_MEMBER_JOINED).toBe(
            "xmpp.muc_lobby_member_joined",
        );
        expect(XMPPEvents.MUC_LOBBY_MEMBER_UPDATED).toBe(
            "xmpp.muc_lobby_member_updated",
        );
        expect(XMPPEvents.MUC_LOBBY_MEMBER_LEFT).toBe(
        ...
        expect(XMPPEvents.MUC_MEMBERS_ONLY_CHANGED).toBe(
            "xmpp.muc_members_only_changed",
        );
        expect(XMPPEvents.MUC_VISITORS_SUPPORTED_CHANGED).toBe(
            "xmpp.muc_visitors_supported_changed",
        );
        ...
        expect(XMPPEvents.PARTICIPANT_VIDEO_TYPE_CHANGED).toBe(
            "xmpp.video_type",
        );
        expect(XMPPEvents.PARTICIPANT_FEATURES_CHANGED).toBe(
            "xmpp.participant_features_changed",
        );
        expect(XMPPEvents.PASSWORD_REQUIRED).toBe("xmpp.password_required");
        expect(XMPPEvents.PHONE_NUMBER_CHANGED).toBe(
        ...
        expect(XMPPEvents.SPEAKER_STATS_RECEIVED).toBe(
            "xmpp.speaker_stats_received",
        );
        expect(XMPPEvents.CONFERENCE_TIMESTAMP_RECEIVED).toBe(
            "xmpp.conference_timestamp_received",
        );
        expect(XMPPEvents.AV_MODERATION_APPROVED).toBe(
            "xmpp.av_moderation.approved",
        );
        expect(XMPPEvents.AV_MODERATION_REJECTED).toBe(
            "xmpp.av_moderation.rejected",
        );
        expect(XMPPEvents.AV_MODERATION_RECEIVED).toBe(
            "xmpp.av_moderation.received",
        );
        expect(XMPPEvents.AV_MODERATION_CHANGED).toBe(
            "xmpp.av_moderation.changed",
        );
        expect(XMPPEvents.AV_MODERATION_PARTICIPANT_APPROVED).toBe(
            "xmpp.av_moderation.participant.approved",
        );
        expect(XMPPEvents.AV_MODERATION_PARTICIPANT_REJECTED).toBe(
            "xmpp.av_moderation.participant.rejected",
        );
        expect(XMPPEvents.BREAKOUT_ROOMS_MOVE_TO_ROOM).toBe(
            "xmpp.breakout-rooms.move-to-room",
        );
        expect(XMPPEvents.BREAKOUT_ROOMS_EVENT).toBe(
            "xmpp.breakout-rooms.event",
        );
        expect(XMPPEvents.BREAKOUT_ROOMS_UPDATED).toBe(
            "xmpp.breakout-rooms.updated",
        );
        expect(XMPPEvents.ROOM_METADATA_EVENT).toBe("xmpp.room-metadata.event");
        expect(XMPPEvents.ROOM_METADATA_UPDATED).toBe(
            "xmpp.room-metadata.updated",
        );
        expect(XMPPEvents.START_MUTED_FROM_FOCUS).toBe(
        ...
        expect(XMPPEvents.TRANSCRIPTION_STATUS_CHANGED).toBe(
            "xmpp.transcription_status_changed",
        );
        expect(XMPPEvents.TRANSPORT_INFO).toBe("xmpp.transportinfo.jingle");
        expect(XMPPEvents.VIDEO_SIP_GW_AVAILABILITY_CHANGED).toBe(
            "xmpp.videoSIPGWAvailabilityChanged",
        );
        expect(XMPPEvents.VIDEO_SIP_GW_SESSION_STATE_CHANGED).toBe(
            "xmpp.videoSIPGWSessionStateChanged",
        );
        expect(XMPPEvents.ICE_CONNECTION_STATE_CHANGED).toBe(
            "xmpp.ice_connection_state_changed",
        );
        expect(XMPPEvents.JSON_MESSAGE_RECEIVED).toBe(
            "xmmp.json_message_received",
        );
    });
