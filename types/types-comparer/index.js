...
const normalisedPathName =
    path.normalize(path.resolve(dirname)).replace(/\\/g, "/") + "/";
...
glob(normalisedPathName + "**/*.d.ts", null, (error, files) => {
    ...
    for (const sourceFile of program.getSourceFiles()) {
        ...
        const serializeMember = (symbol) =>
            `${serializeModifierFlags(symbol)}${symbol.getName()}: ${normaliseImportedType(checker.typeToString(checker.getTypeOfSymbolAtLocation(symbol, symbol.valueDeclaration)))}`;

        const serializeSignature = (signature) =>
            signature.parameters
                .map((symbol) => serializeMember(symbol))
                .join(", ");
        ...
        const visit = (node) => {
            ...
            if (ts.isClassDeclaration(node) && node.name) {
                ...
                if (symbol) {
                    ...
                    output.push({
                        fileName: normaliseFilename(sourceFile.fileName),
                        ...definition,
                    });
                }
