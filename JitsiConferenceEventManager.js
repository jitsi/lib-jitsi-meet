...
JitsiConferenceEventManager.prototype.setupChatRoomListeners = function () {
    ...
    this.chatRoomForwarder = new EventEmitterForwarder(
        chatRoom,
    ...
    chatRoom.addListener(XMPPEvents.VIDEO_MUTED_BY_FOCUS, (actor) => {
        ...
        conference.rtc
            ...
            .catch((error) => {
                conference.mutedVideoByFocusActor = null;
                logger.warn(
                    "Error while video muting due to focus request",
                    error,
                );
            });
    });
    ...
    chatRoom.addListener(XMPPEvents.MUC_JOINED, () => {
        ...
        Object.keys(chatRoom.connectionTimes).forEach((key) => {
            const event = createConnectionStageReachedEvent(
                `conference_${key}`,
    ...
    chatRoom.addListener(
        ...
        (track, owner, sourceName, videoType) => {
            if (
                track.getParticipantId() !== owner ||
                track.getSourceName() !== sourceName
            ) {
                conference.eventEmitter.emit(
    ...
    this.chatRoomForwarder.forward(
        XMPPEvents.CHAT_ERROR_RECEIVED,
    ...
    chatRoom.addListener(XMPPEvents.SESSION_ACCEPT_TIMEOUT, (jingleSession) => {
        Statistics.sendAnalyticsAndLog(
            createJingleEvent(ACTION_JINGLE_SA_TIMEOUT, {
    ...
    chatRoom.addListener(XMPPEvents.RECORDER_STATE_CHANGED, (session, jid) => {
        if (jid) {
            ...
            const participant =
                conference.getParticipantById(resource) || resource;
    ...
    this.chatRoomForwarder.forward(
        XMPPEvents.VIDEO_SIP_GW_AVAILABILITY_CHANGED,
    ...
    this.chatRoomForwarder.forward(
        XMPPEvents.PHONE_NUMBER_CHANGED,
    ...
    chatRoom.addListener(
        XMPPEvents.SUSPEND_DETECTED,
        conference.onSuspendDetected.bind(conference),
    );

    this.chatRoomForwarder.forward(
        XMPPEvents.MUC_LOCK_CHANGED,
    ...
    // Room metadata.
    this.chatRoomForwarder.forward(
        XMPPEvents.ROOM_METADATA_UPDATED,
...
JitsiConferenceEventManager.prototype.setupRTCListeners = function () {
        ...
        (dominant, previous, silence) => {
            if (
                ...
                if (
                    ...
                    conference.myUserId() === dominant
                ) {
                    // We are the new dominant speaker.
                    conference.xmpp.sendDominantSpeakerEvent(
                ...
                if (conference.lastDominantSpeaker !== dominant) {
                    if (previous && previous.length) {
                        ...
                        if (speakerList.length > SPEAKERS_AUDIO_LEVELS) {
                            speakerList.splice(
                                SPEAKERS_AUDIO_LEVELS,
                                speakerList.length - SPEAKERS_AUDIO_LEVELS,
                            );
                        }
                        conference.statistics &&
    ...
    rtc.addListener(RTCEvents.DATA_CHANNEL_CLOSED, (ev) => {
        conference.eventEmitter.emit(
            JitsiConferenceEvents.DATA_CHANNEL_CLOSED,
            ev,
        );
    });
    ...
    rtc.addListener(RTCEvents.ENDPOINT_STATS_RECEIVED, (from, payload) => {
        ...
        if (participant) {
            conference.eventEmitter.emit(
                JitsiConferenceEvents.ENDPOINT_STATS_RECEIVED,
                participant,
                payload,
            );
        } else {
            logger.warn(
                `Ignoring ENDPOINT_STATS_RECEIVED for a non-existant participant: ${from}`,
            );
        }
    });
...
JitsiConferenceEventManager.prototype.setupXMPPListeners = function () {
    ...
    this._addConferenceXMPPListener(
        XMPPEvents.AV_MODERATION_CHANGED,
        (value, mediaType, actorJid) => {
            const actorParticipant = conference
                .getParticipants()
                .find((p) => p.getJid() === actorJid);

            conference.eventEmitter.emit(
                JitsiConferenceEvents.AV_MODERATION_CHANGED,
                {
                    enabled: value,
                    mediaType,
    ...
    this._addConferenceXMPPListener(
        XMPPEvents.AV_MODERATION_PARTICIPANT_APPROVED,
        (mediaType, jid) => {
            const participant = conference.getParticipantById(
    ...
    this._addConferenceXMPPListener(
        XMPPEvents.AV_MODERATION_PARTICIPANT_REJECTED,
        (mediaType, jid) => {
            const participant = conference.getParticipantById(
                Strophe.getResourceFromJid(jid),
            );
    ...
    this._addConferenceXMPPListener(
        XMPPEvents.AV_MODERATION_APPROVED,
        (value) =>
            conference.eventEmitter.emit(
                JitsiConferenceEvents.AV_MODERATION_APPROVED,
                { mediaType: value },
            ),
    );
    this._addConferenceXMPPListener(
        XMPPEvents.AV_MODERATION_REJECTED,
        (value) => {
            conference.eventEmitter.emit(
                JitsiConferenceEvents.AV_MODERATION_REJECTED,
                { mediaType: value },
            );
        },
    );

    this._addConferenceXMPPListener(XMPPEvents.VISITORS_MESSAGE, (value) =>
        conference.eventEmitter.emit(
            JitsiConferenceEvents.VISITORS_MESSAGE,
            value,
        ),
    );
