...
export default class SDP {
    ...
    updateRemoteSources(sourceMap, isAdd = true) {
        ...
        for (const source of sourceMap.values()) {
            ...
            if (isAdd) {
                ...
                idx = this.media.findIndex(
                    (mLine) =>
                        mLine.includes(`m=${mediaType}`) &&
                        !mLine.includes("a=ssrc"),
                );
                if (!this.isP2P || idx === -1) {
                ...
                idx = this.media.findIndex((mLine) =>
                    mLine.includes(`a=ssrc:${ssrcList[0]}`),
                );
            ...
            if (isAdd) {
                ...
                groups?.forEach((group) => {
                    this.media[idx] +=
                        `a=ssrc-group:${group.semantics} ${group.ssrcs.join(" ")}\r\n`;
                });
            } else {
                ssrcList.forEach((ssrc) => {
                    this.media[idx] = this.media[idx].replace(
                ...
                if (!this.isP2P) {
                    ...
                    const { media, port } = SDPUtil.parseMLine(
                        this.media[idx].split("\r\n")[0],
                    );
    ...
    fromJingle(jingle) {
        ...
        newSession.media.forEach((mLine) => {
            ...
            mLine.ssrcs.forEach((ssrc, idx) => {
                ...
                if (
                    newMedia.some((mline) =>
                        mline.ssrcs?.some((source) => source.id === ssrc.id),
                    )
                ) {
                    return;
                }
                ...
                const group = mLine.ssrcGroups?.find((g) =>
                    g.ssrcs.includes(ssrcId),
                );

                if (group) {
                    newMline.ssrcs.push(ssrc);
                    const otherSsrc = group.ssrcs
                        .split(" ")
                        .find((s) => s !== ssrcId);

                    if (otherSsrc) {
                        const otherSource = mLine.ssrcs.find(
                            (source) => source.id.toString() === otherSsrc,
                        );
    ...
    getMediaSsrcMap() {
        ...
        this.media.forEach((mediaItem, mediaindex) => {
            ...
            const isRecvOnly = SDPUtil.findLine(
                mediaItem,
    ...
    jingle2media(content) {
        ...
        const transport = content.find(
            `>transport[xmlns='${XEP.ICE_UDP_TRANSPORT}']`,
        );
        let sdp = "";
        const sctp = transport.find(
            `>sctpmap[xmlns='${XEP.SCTP_DATA_CHANNEL}']`,
        );
        ...
        if (transport.length) {
            ...
            transport
                .find(`>fingerprint[xmlns='${XEP.DTLS_SRTP}']`)
                .each((_, fingerprint) => {
        ...
        desc.find(">payload-type").each((_, payloadType) => {
            ...
            sdp += this.rtcpFbFromJingle(
                $(payloadType),
                payloadType.getAttribute("id"),
            );
        });

        sdp += this.rtcpFbFromJingle(desc, "*");

        desc.find(`>rtp-hdrext[xmlns='${XEP.RTP_HEADER_EXTENSIONS}']`).each(
            (_, hdrExt) => {
        ...
        if (
            desc.find(
                `>extmap-allow-mixed[xmlns='${XEP.RTP_HEADER_EXTENSIONS}']`,
            ).length > 0
        ) {
            sdp += "a=extmap-allow-mixed\r\n";
        }

        desc.find(`>ssrc-group[xmlns='${XEP.SOURCE_ATTRIBUTES}']`).each(
            (_, ssrcGroup) => {
        ...
        desc.find(`>source[xmlns='${XEP.SOURCE_ATTRIBUTES}']`).each(
            (_, source) => {
    ...
    rtcpFbFromJingle(elem, payloadtype) {
        let sdp = "";
        const feedbackElementTrrInt = elem.find(
            `>rtcp-fb-trr-int[xmlns='${XEP.RTP_FEEDBACK}']`,
        );
        ...
        const feedbackElements = elem.find(
            `>rtcp-fb[xmlns='${XEP.RTP_FEEDBACK}']`,
        );
    ...
    toJingle(elem, thecreator) {
        SDPUtil.findLines(this.session, "a=group:").forEach((line) => {
            ...
            if (this.isP2P) {
                mediaTypes = this.media.map((mediaItem) =>
                    SDPUtil.parseMID(SDPUtil.findLine(mediaItem, "a=mid:")),
                );
            }
            mediaTypes.forEach((type) =>
                elem.c("content", { name: type }).up(),
            );
            elem.up();
        });
        ...
        this.media.forEach((mediaItem, i) => {
            const mline = SDPUtil.parseMLine(mediaItem.split("\r\n")[0]);
            const mediaType =
                mline.media === MediaType.APPLICATION ? "data" : mline.media;
            ...
            const contents = $(elem.tree()).find(
                `content[name='${mediaType}']`,
            );
            ...
            for (const content of contents) {
                ...
                if (ssrc && !(isRecvOnly && browser.isFirefox())) {
                    ...
                    for (const [availableSsrc, ssrcParameters] of ssrcMap) {
                        const sourceName =
                            SDPUtil.parseSourceNameLine(ssrcParameters);
                        const videoType =
                            SDPUtil.parseVideoTypeLine(ssrcParameters);
                    ...
                    const ssrcGroupLines = SDPUtil.findLines(
                        mediaItem,
                        "a=ssrc-group:",
                    );

                    ssrcGroupLines.forEach((line) => {
                        const { semantics, ssrcs } =
                            SDPUtil.parseSSRCGroupLine(line);
            ...
            if (
                ...
                typeof this.initialLastN === "number"
            ) {
            ...
            if ([MediaType.AUDIO, MediaType.VIDEO].includes(mediaType)) {
                ...
                mline.fmt.forEach((format) => {
                    ...
                    const afmtpline = SDPUtil.findLine(
                        mediaItem,
                        `a=fmtp:${format}`,
                    );
                    ...
                    if (afmtpline) {
                        ...
                        fmtpParameters.forEach((param) =>
                            elem.c("parameter", param).up(),
                        );
                    }
                ...
                if (ridLines.length && browser.usesRidsForSimulcast()) {
                    ...
                    const rids = ridLines.map(
                        (ridLine) => ridLine.split(":")[1].split(" ")[0],
                    );
                    ...
                    const unifiedSimulcast = SDPUtil.findLine(
                        mediaItem,
                        "a=simulcast:",
                    );
                ...
                const extmapLines = SDPUtil.findLines(
                    mediaItem,
                    "a=extmap:",
                    this.session,
                );
            ...
            if (SDPUtil.findLine(mediaItem, `a=${MediaDirection.SENDRECV}`)) {
                ...
                SDPUtil.findLine(mediaItem, `a=${MediaDirection.SENDONLY}`)
            ) {
                elem.attrs({ senders: "initiator" });
            } else if (
                SDPUtil.findLine(mediaItem, `a=${MediaDirection.RECVONLY}`)
            ) {
                elem.attrs({ senders: "responder" });
            } else if (
                SDPUtil.findLine(mediaItem, `a=${MediaDirection.INACTIVE}`)
            ) {
                elem.attrs({ senders: "none" });
            }
            ...
            if (
                mline.port === "0" &&
                !SDPUtil.findLine(mediaItem, "a=bundle-only", this.session)
            ) {
    ...
    transportToJingle(mediaIndex, elem) {
        ...
        const sctpport = SDPUtil.findLine(
            this.media[mediaIndex],
            "a=sctp-port:",
            this.session,
        );
        const sctpmap = SDPUtil.findLine(
            this.media[mediaIndex],
            "a=sctpmap:",
            this.session,
        );
        ...
        const fingerprints = SDPUtil.findLines(
            this.media[mediaIndex],
            "a=fingerprint:",
            this.session,
        );
        ...
        fingerprints.forEach((line) => {
            ...
            const setupLine = SDPUtil.findLine(
                this.media[mediaIndex],
                "a=setup:",
                this.session,
            );
        ...
        const iceParameters = SDPUtil.iceparams(
            this.media[mediaIndex],
            this.session,
        );
        ...
        if (iceParameters) {
            ...
            const candidateLines = SDPUtil.findLines(
                this.media[mediaIndex],
                "a=candidate:",
                this.session,
            );
