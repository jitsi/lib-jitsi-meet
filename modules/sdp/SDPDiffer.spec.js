...
describe("SDPDiffer", () => {
    ...
    describe("toJingle", () => {
        it("should include source names in added/removed sources", () => {
            ...
            const newToOldDiff = new SDPDiffer(
                new SDP(testSdpNew),
                new SDP(testSdpOld),
            );
            ...
            const removedAudioSources =
                sourceRemoveIq.nodeTree.querySelectorAll(
                    "description[media='audio']>source",
                );

            expect(removedAudioSources[0].getAttribute("name")).toBe(
                "a8f7g30-a0",
            );

            const oldToNewDiff = new SDPDiffer(
                new SDP(testSdpOld),
                new SDP(testSdpNew),
            );
            const sourceAddIq = $iq({}).c("jingle", { action: "source-add" });

            oldToNewDiff.toJingle(sourceAddIq);

            const addedVideoSources = sourceAddIq.nodeTree.querySelectorAll(
                "description[media='video']>source",
            );
            const addedVideoSourceGroups =
                sourceAddIq.nodeTree.querySelectorAll(
                    "description[media='video']>ssrc-group",
                );

            expect(addedVideoSources.length).toBe(2);
            expect(addedVideoSources[0].getAttribute("name")).toBe(
                "a8f7g30-v0",
            );
            expect(addedVideoSources[1].getAttribute("name")).toBe(
                "a8f7g30-v0",
            );
        ...
        it("should send source-remove/source-add when ssrc changes", () => {
            ...
            const addedAudioSources = sourceAddIq.nodeTree.querySelectorAll(
                "description[media='audio']>source",
            );
            const addedVideoSources = sourceAddIq.nodeTree.querySelectorAll(
                "description[media='video']>source",
            ...
            expect(addedAudioSources[0].getAttribute("name")).toBe(
                "a8f7g30-a0",
            );
    ...
    describe("getNewMedia", () => {
        ...
        it(" should generate sources for source-remove and source-add when SSRC changes", () => {
            ...
            const newSdp = new SDP(
                SampleSdpStrings.simulcastDifferentSsrcSdpStr,
            );
