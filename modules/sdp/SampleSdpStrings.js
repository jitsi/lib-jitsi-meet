...
const simulcastSdpStr =
    baseSessionSdp +
    baseAudioMLineSdp +
    simulcastVideoMLineSdp +
    baseDataMLineSdp;

const simulcastDifferentSsrcSdpStr =
    baseSessionSdp +
    baseAudioMLineSdpDifferentSSRC +
    simulcastVideoMLineSdp +
    baseDataMLineSdp;

// A full sdp string representing a remote client doing simucast when RTX is not negotiated with the jvb.
const simulcastNoRtxSdpStr =
    baseSessionSdp + baseAudioMLineSdp + simulcastVideoMLineNoRtxSdp;

// A full sdp string representing a client doing simulcast and rtx
const simulcastRtxSdpStr =
    baseSessionSdp +
    baseAudioMLineSdp +
    simulcastRtxVideoMLineSdp +
    baseDataMLineSdp;

// A full sdp string representing a client doing simulcast and rtx with a different track and SSRCs.
const simulcastRtxSdpTrackReplaced =
    baseSessionSdp + baseAudioMLineSdp + simulcastRtxVideoMLineSdpReplacedTrack;

// A full sdp string representing a client doing a single video stream
const plainVideoSdpStr =
    baseSessionSdp + baseAudioMLineSdp + plainVideoMLineSdp + baseDataMLineSdp;

// A full sdp string representing a client doing a single video stream with rtx
const rtxVideoSdpStr =
    baseSessionSdp + baseAudioMLineSdp + rtxVideoMLineSdp + baseDataMLineSdp;

// A full sdp string representing a client doing a single video stream with multiple codec options
const multiCodecVideoSdpStr =
    baseSessionSdp +
    baseAudioMLineSdp +
    multiCodecVideoMLine +
    baseDataMLineSdp;
...
const flexFecSdpStr =
    baseSessionSdp +
    baseAudioMLineSdp +
    flexFecVideoMLineSdp +
    baseDataMLineSdp;

// A full sdp string representing a client that doesn't have local sources added on Firefox.
const recvOnlySdpStr =
    baseSessionSdp + recvOnlyAudioMline + recvOnlyVideoMline + baseDataMLineSdp;

// A full sdp string representing a client that doesn't have local sources added on Chrome.
const recvOnlySdpStrChrome =
    baseSessionSdp +
    recvOnlyAudioMlineChrome +
    recvOnlyVideoMlineChrome +
    baseDataMLineSdp;
