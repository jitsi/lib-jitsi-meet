...
describe("TransformSdpsForUnifiedPlan", () => {
    ...
    describe("StripSsrcs", () => {
        it("should strip ssrcs from an sdp with no msid (i.e., recvonly transceivers)", () => {
            ...
            const transformedDesc = localSdpMunger.transformStreamIdentifiers(
                desc,
                {},
            );
        ...
        describe("should strip cname, label and mslabel from an sdp with msid", () => {
            ...
            const transformStreamIdentifiers = () => {
                ...
                const transformedDesc =
                    localSdpMunger.transformStreamIdentifiers(desc, ssrcMap);
                const newSdp = transform.parse(transformedDesc.sdp);
            ...
            it("with source name signaling enabled (injected source name)", () => {
                ...
                expect(audioSsrcs.length).toEqual(
                    1 + 1 /* injected source name */,
                );
                expect(videoSsrcs.length).toEqual(
                    3 + 3 /* injected source name into each ssrc */,
                );
            });
        });
    });
        ...
        it("should add endpointId to msid", () => {
            ...
            for (const ssrcLine of videoSsrcs) {
                if (ssrcLine.attribute === "msid") {
                    ...
                    expect(msid).toBe(
                        `${localEndpointId}-video-0-${tpc.id} bdbd2c0a-7959-4578-8db5-9a6a1aec4ecf-${tpc.id}`,
                    );
                }
            }
        });
...
describe("Transform msids for source-name signaling", () => {
    ...
    const transformStreamIdentifiers = () => {
        ...
        ssrcMap.set("sRdpsdg-v0", {
            ssrcs: [
                1757014965, 984899560, 1479742055, 855213044, 1089111804,
                2963867077,
            ],
            msid: "sRdpsdg-video-0",
        });
        ...
        audioMsidLine = getSsrcLines(newSdp, "audio").find(
            (ssrc) => ssrc.attribute === "msid",
        )?.value;
        audioMsid = audioMsidLine.split(" ")[0];
        videoMsidLine = getSsrcLines(newSdp, "video").find(
            (ssrc) => ssrc.attribute === "msid",
        )?.value;
