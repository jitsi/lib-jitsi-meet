...
const SDPUtil = {
    ...
    iceparams(mediadesc, sessiondesc) {
        ...
        if (
            (ufrag = SDPUtil.findLine(
                mediadesc,
                "a=ice-ufrag:",
                sessiondesc,
            )) &&
            (pwd = SDPUtil.findLine(mediadesc, "a=ice-pwd:", sessiondesc))
        ) {
            data = {
    ...
    parseMSIDAttribute(ssrcLines) {
        ...
        const v = msidLine.substring(
            msidLine.indexOf(" msid:") + 6 /* the length of ' msid:' */,
        );
    ...
    buildRTPMap(el) {
        let line = `a=rtpmap:${el.getAttribute("id")} ${el.getAttribute("name")}/${el.getAttribute(
            "clockrate",
        )}`;
    ...
    parseSourceNameLine(ssrcLines) {
        const sourceNameLine = ssrcLines.find(
            (ssrcSdpLine) => ssrcSdpLine.indexOf(" name:") > 0,
        );
    ...
    parseVideoTypeLine(ssrcLines) {
        const s = " videoType:";
        const videoTypeLine = ssrcLines.find(
            (ssrcSdpLine) => ssrcSdpLine.indexOf(s) > 0,
        );
    ...
    candidateToJingle(line) {
        ...
        for (let i = 8; i < elems.length; i += 2) {
            switch (elems[i]) {
                ...
                default: // TODO
                    logger.debug(
                        `not translating "${elems[i]}" = "${elems[i + 1]}"`,
                    );
            }
        }
        candidate.network = "1";
    ...
    parsePrimaryVideoSsrc(videoMLine) {
        ...
        if (numSsrcs === 1) {
            ...
            const fidGroup = videoMLine.ssrcGroups.find(
    ...
    preferCodec(mline, codecName, sortPayloadTypes = false) {
        ...
        if (matchingPayloadTypes) {
            if (sortPayloadTypes && codecName === CodecMimeType.H264) {
                ...
                const payloadsWithMode0 = matchingPayloadTypes.filter(
                    (payload) => {
                        const fmtp = mline.fmtp.find(
                ...
                for (const pt of payloadsWithMode0.reverse()) {
                    const idx = matchingPayloadTypes.findIndex(
            ...
            logger.error(
                `No matching RTP payload type found for ${codecName}, failed to set preferred codecs`,
            );
        }
    },
