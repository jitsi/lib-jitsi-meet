...
describe("/modules/detection/DetectionEvents members", () => {
    ...
    it("known members", () => {
        ...
        expect(DetectionEvents.DETECTOR_STATE_CHANGE).toBe(
            "detector_state_change",
        );
        expect(DetectionEvents.AUDIO_INPUT_STATE_CHANGE).toBe(
            "audio_input_state_changed",
        );
        expect(DetectionEvents.NO_AUDIO_INPUT).toBe("no_audio_input_detected");
        expect(DetectionEvents.VAD_NOISY_DEVICE).toBe(
            "detection.vad_noise_device",
        );
        expect(DetectionEvents.VAD_REPORT_PUBLISHED).toBe(
            "vad-report-published",
        );
        expect(DetectionEvents.VAD_SCORE_PUBLISHED).toBe(
            "detection.vad_score_published",
        );
        expect(DetectionEvents.VAD_TALK_WHILE_MUTED).toBe(
