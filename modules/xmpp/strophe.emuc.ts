...
import { exists, findAll, getAttribute } from '../util/XMLUtils';
...
export default class MucConnectionPlugin extends ConnectionPluginListenable {
    ...
    onPresence(pres: Element): boolean {
        ...
        if (exists(pres, 'x[*|xmlns="http://jabber.org/protocol/muc#user"] status[code="201"]')) {
    ...
    onVisitors(iq: Element): boolean {
        ...
        // Use *|xmlns to match xmlns attributes across any namespace (CSS Selectors Level 3)
        const visitors = findAll(iq, 'visitors[*|xmlns="jitsi:visitors"]');
        const response = findAll(iq, 'promotion-response');

        if (visitors.length && response.length) {
            if (String(getAttribute(response[0], 'allow')).toLowerCase() === 'true') {
                logger.info('Promotion request accepted. Redirected to main room.');
                this.xmpp.eventEmitter.emit(
                    CONNECTION_REDIRECTED, undefined, getAttribute(visitors[0], 'focusjid'), getAttribute(response[0], 'username'));
            } else {
