...
export default class AVModeration {
    ...
    enable(state, mediaType) {
        ...
        if (state === this._moderationEnabledByType[mediaType]) {
            logger.warn(
                `Moderation already in state:${state} for mediaType:${mediaType}`,
            );
        ...
        if (newWhitelists) {
            const oldList =
                media === MediaType.AUDIO
                    ? this._whitelistAudio
                    : this._whitelistVideo;
            const newList = Array.isArray(newWhitelists[media])
                ? newWhitelists[media]
                : [];
