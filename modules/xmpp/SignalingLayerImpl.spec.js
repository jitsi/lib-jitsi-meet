...
import SignalingLayerImpl, {
    SOURCE_INFO_PRESENCE_ELEMENT,
} from "./SignalingLayerImpl";
...
describe("SignalingLayerImpl", () => {
    describe("setTrackMuteStatus advertises the track muted status in the chat room", () => {
        describe("with source name signaling enabled", () => {
            ...
            beforeEach(() => {
                ...
                // No tracks yet
                expect(chatRoom.addOrReplaceInPresence).toHaveBeenCalledWith(
                    SOURCE_INFO_PRESENCE_ELEMENT,
                    INITIAL_SOURCE_INFO,
                );
            });
            it("for audio track", () => {
                const audioSourceName = getSourceNameForJitsiTrack(
                    endpointId,
                    MediaType.AUDIO,
                    0,
                );
            ...
            it("for video track", () => {
                const videoSourceName = getSourceNameForJitsiTrack(
                    endpointId,
                    MediaType.VIDEO,
                    0,
                );
    ...
    describe("setTrackVideoType", () => {
        ...
        it("sends video type in chat room presence", () => {
            ...
            signalingLayer.setTrackVideoType(
                videoSourceName,
                VideoType.DESKTOP,
            );
            expect(chatRoom.addOrReplaceInPresence).toHaveBeenCalledWith(
    ...
    describe("should emit muted/video type events based on presence", () => {
        describe("with:  sourceNameSignaling: true", () => {
            ...
            it("from a user with SourceInfo and ssrc-rewriting disabled", () => {
                FeatureFlags.init({ ssrcRewritingEnabled: false });
                const emitterSpy = spyOn(signalingLayer.eventEmitter, "emit");
                const sourceInfo = {
                    "12345678-a0": {
                        muted: true,
    ...
    describe("getPeerMediaInfo", () => {
        describe("with:  sourceNameSignaling: true", () => {
            ...
            it("will provide default value if only empty source info was sent so far", () => {
                ...
                const audioPeerMediaInfo = signalingLayer.getPeerMediaInfo(
                    endpointId,
                    MediaType.AUDIO,
                );
                ...
                const videoPeerMediaInfo = signalingLayer.getPeerMediaInfo(
                    endpointId,
                    MediaType.VIDEO,
                );
            ...
            describe("will read from SourceInfo if available", () => {
                it("for audio", () => {
                    ...
                    const peerMediaInfo = signalingLayer.getPeerMediaInfo(
                        endpointId,
                        MediaType.AUDIO,
                        "12345678-a0",
                    );
    ...
    describe("will remove source info(cleanup corner cases)", () => {
        ...
        it("when participant leaves", () => {
            ...
            expect(
                signalingLayer.getPeerSourceInfo(endpointId, "12345678-v0"),
            ).toBeDefined();
