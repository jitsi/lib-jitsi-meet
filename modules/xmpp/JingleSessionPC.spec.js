...
describe("JingleSessionPC", () => {
    ...
    describe("send/receive video constraints w/ source-name", () => {
        it("sends content-modify with recv frame size", () => {
            ...
            return new Promise((resolve, reject) => {
                ...
                expect(jingleSession.getState()).toBe(
        ...
        it("fires an event when remote peer sends content-modify", () => {
            ...
            const remoteVideoConstraintsListener = () => {
                remoteSourcesRecvMaxFrameHeight =
                    jingleSession.getRemoteSourcesRecvMaxFrameHeight();
            };
            ...
            return new Promise((resolve, reject) => {
                ...
                jingleSession.modifyContents(
                    createContentModifyForSourceNames(),
                );
    ...
    describe("_processSourceAddOrRemove", () => {
        let peerconnection,
            ...
            setSsrcOwnerSpy,
            sourceInfo,
            updateRemoteSourcesSpy;

        beforeEach(() => {
            peerconnection = jingleSession.peerconnection;
            setSsrcOwnerSpy = spyOn(
                jingleSession._signalingLayer,
                "setSSRCOwner",
            );
            removeSsrcOwnersSpy = spyOn(
                jingleSession._signalingLayer,
                "removeSSRCOwners",
            );
            updateRemoteSourcesSpy = spyOn(
                peerconnection,
                "updateRemoteSources",
            );
        });
        it("should handle no sources", () => {
            ...
            sourceInfo = jingleSession._processSourceMapFromJingle(
                sourceAddElem,
                true,
            );
        ...
        it("should handle a single source", () => {
            ...
            expect(updateRemoteSourcesSpy).toHaveBeenCalledWith(
                sourceInfo,
                true,
            );
        ...
        it("should handle multiple ssrcs belonging to the same source", () => {
            ...
            expect(sourceInfo.size).toBe(1);
            expect(sourceInfo.get("source1").ssrcList).toEqual([
                "1234",
                "5678",
            ]);
            expect(sourceInfo.get("source1").msid).toBe("stream1");
        ...
        it("should handle multiple ssrcs belonging to different sources", () => {
            ...
            expect(sourceInfo.get("source1").mediaType).toBe("video");
            expect(sourceInfo.get("source2").ssrcList).toEqual([
                "4321",
                "8765",
            ]);
            ...
            expect(removeSsrcOwnersSpy).toHaveBeenCalledWith([
                1234, 5678, 4321, 8765,
            ]);
            expect(updateRemoteSourcesSpy).toHaveBeenCalledWith(
                sourceInfo,
