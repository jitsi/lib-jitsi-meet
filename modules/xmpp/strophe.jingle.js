...
export default class JingleConnectionPlugin extends ConnectionPlugin {
     ...
     */
    onJingle(iq) {
        // The iq parameter is already a DOM Element in Strophe handlers
        const jingleElement = findAll(iq, 'jingle')[0];
        const sid = jingleElement ? getAttribute(jingleElement, 'sid') : null;
        const action = jingleElement ? getAttribute(jingleElement, 'action') : null;
        ...
        case 'session-accept': {
            ...
            for (const content of contents) {
                const descriptionElement = findAll(content, 'description')[0];
                const ssrc = descriptionElement ? getAttribute(descriptionElement, 'ssrc') : null;
        ...
        case 'transport-info': {
            const transportElements = findAll(iq, 'jingle>content>transport');
            const candidates = transportElements.length > 0 ? _parseIceCandidates(transportElements[0]) : [];
        ...
        case 'session-terminate': {
            ...
            const reasonElements = findAll(iq, ':scope > jingle > reason');

            if (reasonElements.length) {
                const firstReasonChild = reasonElements[0].children[0];

                reasonCondition = firstReasonChild ? firstReasonChild.tagName : null;

                const reasonTextElement = findAll(iq, ':scope > jingle > reason > text')[0];

                reasonText = reasonTextElement ? getText(reasonTextElement) : null;
            }
        ...
        case 'transport-replace':
            ...
            break;
        case 'source-add':
            sess.addRemoteStream(findAll(iq, 'jingle>content'));
            break;
        case 'source-remove':
            sess.removeRemoteStream(findAll(iq, 'jingle>content'));
            break;
        default:
