...
var SdpConsistency = {
    cachedPrimarySsrc: null,
    cachedPrimaryRtxSsrc: null,
    
    clearSsrcCache: function() {
        this.cachedPrimarySsrc = null;
        this.cachedPrimaryRtxSsrc = null;
    },

    setPrimarySsrc: function(primarySsrc) {
        this.cachedPrimarySsrc = primarySsrc;
    },

    _getPrimarySsrc: function(videoMLine) {
        if (!videoMLine.ssrcs) {
            return;
        }
    ...
    _getRtxSsrc: function(videoMLine, primarySsrc) {
        if (videoMLine.ssrcGroups) {
            return videoMLine
                .ssrcGroups
    ...
    makeVideoPrimarySsrcsConsistent: function (sdpStr) {
        ...
        if (videoMLine.direction === "recvonly") {
            ...
            let newPrimarySsrc = this._getPrimarySsrc(videoMLine);
            if (!newPrimarySsrc) {
                console.log("BB: consistency couldn't parse new primary ssrc");
                return sdpStr;
            }
            let newPrimaryRtxSsrc = 
                this._getRtxSsrc(videoMLine, newPrimarySsrc);
            if (!this.cachedPrimarySsrc) {
                this.cachedPrimarySsrc = newPrimarySsrc;
                this.cachedPrimaryRtxSsrc = newPrimaryRtxSsrc;
                console.log("BB: consistency caching primary ssrc " + 
                    this.cachedPrimarySsrc + " and rtx " + 
                    this.cachedPrimaryRtxSsrc);
            } else {
                console.log("BB: consistency replacing new ssrc " + 
