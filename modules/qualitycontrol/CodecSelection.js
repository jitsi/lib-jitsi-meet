...
export class CodecSelection {
    ...
    constructor(conference, options) {
        ...
        for (const connectionType of Object.keys(options)) {
            ...
            const supportedCodecs = new Set(
                this._getSupportedVideoCodecs(connectionType),
            );
            ...
            if (preferenceOrder) {
                preferenceOrder = preferenceOrder.map((codec) =>
                    codec.toLowerCase(),
                );

                // Select all codecs that are supported by the browser.
                selectedOrder = preferenceOrder.filter((codec) =>
                    supportedCodecs.has(codec),
                );
                ...
                if (disabledCodec && disabledCodec !== CodecMimeType.VP8) {
                    selectedOrder = selectedOrder.filter(
                        (codec) => codec !== disabledCodec,
                    );
                }

                const index = selectedOrder.findIndex(
                    (codec) => codec === preferredCodec,
                );
            ...
            logger.info(
                `Codec preference order for ${connectionType} connection is ${selectedOrder}`,
            );
            ...
            if (
                screenshareCodec &&
                supportedCodecs.has(screenshareCodec.toLowerCase())
            ) {
                this.screenshareCodec[connectionType] =
                    screenshareCodec.toLowerCase();
            }
        }
    }
    ...
    _getSupportedVideoCodecs(connectionType) {
        const videoCodecMimeTypes =
            browser.isMobileDevice() && connectionType === "p2p"
        ...
        const supportedCodecs = videoCodecMimeTypes.filter((codec) =>
            (
                window.RTCRtpReceiver?.getCapabilities?.(MediaType.VIDEO)
                    ?.codecs ?? []
            ).some(
                (supportedCodec) =>
                    supportedCodec.mimeType.toLowerCase() ===
                    `${MediaType.VIDEO}/${codec}`,
            ),
        );
    ...
    selectPreferredCodec(mediaSession) {
        const session = mediaSession
            ? mediaSession
            : this.conference.jvbJingleSession;
        ...
        const remoteParticipants = this.conference
            .getParticipants()
            .map((participant) => participant.getId());
        const remoteCodecsPerParticipant = remoteParticipants?.map((remote) => {
            const peerMediaInfo = session._signalingLayer.getPeerMediaInfo(
                remote,
                MediaType.VIDEO,
            );
        ...
        this.visitorCodecs.length &&
            remoteCodecsPerParticipant.push(this.visitorCodecs);

        const selectedCodecOrder = localPreferredCodecOrder.reduce(
            (acc, localCodec) => {
                ...
                for (const remoteCodecs of remoteCodecsPerParticipant) {
                    ...
                    if (remoteCodecs.length) {
                        codecNotSupportedByRemote =
                            codecNotSupportedByRemote ||
                            !remoteCodecs.find(
                                (participantCodec) =>
                                    participantCodec === localCodec,
                            );
                    }
                }
        ...
        if (!selectedCodecOrder.length) {
            logger.warn(
                "Invalid codec list generated because of a user joining/leaving the call",
            );
    ...
    changeCodecPreferenceOrder(localTrack, codec) {
        ...
        const codecsByVideoType = VIDEO_CODECS_BY_COMPLEXITY[videoType].filter(
            (val) =>
                Boolean(
                    codecOrder.find((supportedCodec) => supportedCodec === val),
                ),
        );
        ...
        if (videoType === VideoType.CAMERA) {
            ...
            logger.info(
                `QualityController - switching camera codec to ${newCodec} because of cpu restriction`,
            );
        } else {
            this.screenshareCodec[connectionType] = newCodec;
            logger.info(
                `QualityController - switching screenshare codec to ${newCodec} because of cpu restriction`,
            );
        }
