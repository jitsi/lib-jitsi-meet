...
describe("RED", () => {
    ...
    describe("addRedundancy with a redundancy of 1", () => {
        ...
        it("adds redundancy on the first packet", () => {
            ...
            expect(spy.calls.argsFor(0)[0].data).toEqual(
                new Uint8Array([0x6f, 0x00]).buffer,
            );
        });
        ...
        it("adds redundancy on the first and second packet", () => {
            ...
            expect(spy.calls.argsFor(0)[0].data).toEqual(
                new Uint8Array([0x6f, 0xde]).buffer,
            );
            expect(spy.calls.argsFor(1)[0].data).toEqual(
                new Uint8Array([0xef, 0x0f, 0x00, 0x01, 0x6f, 0xde, 0xad, 0xbe])
                    .buffer,
            );
        });
        ...
        it("does not add redundancy for the first packet on the third packet", () => {
            ...
            expect(spy.calls.argsFor(2)[0].data).toEqual(
                new Uint8Array([
                    0xef, 0x0f, 0x00, 0x02, 0x6f, 0xad, 0xbe, 0xef, 0xff, 0xff,
                ]).buffer,
            );
        });
        ...
        it("does not add redundancy for DTX packets with a 400ms timestamp gap", () => {
            ...
            expect(spy.calls.argsFor(1)[0].data).toEqual(
                new Uint8Array([0x6f, 0xad, 0xbe]).buffer,
            );
            expect(spy.calls.argsFor(2)[0].data).toEqual(
                new Uint8Array([
                    0xef, 0x0f, 0x00, 0x02, 0x6f, 0xad, 0xbe, 0xef, 0xff, 0xff,
    ...
    describe("addRedundancy with a redundancy of 2", () => {
        ...
        it("adds redundancy on the first, second and third packet", () => {
            ...
            expect(spy.calls.argsFor(2)[0].data).toEqual(
                new Uint8Array([
                    0xef, 0x1e, 0x00, 0x01, 0xef, 0x0f, 0x00, 0x02, 0x6f, 0xde,
                    0xad, 0xbe, 0xef, 0xff, 0xff,
                ]).buffer,
            );
        });
        ...
        it("does not add redundancy for the first packet on the fourth packet", () => {
            ...
            expect(spy.calls.argsFor(3)[0].data).toEqual(
                new Uint8Array([
                    0xef, 0x1e, 0x00, 0x02, 0xef, 0x0f, 0x00, 0x03, 0x6f, 0xad,
                    0xbe, 0xef, 0xff, 0xff, 0xfa, 0x1f, 0xfa, 0x1f,
                ]).buffer,
            );
        });
    });
