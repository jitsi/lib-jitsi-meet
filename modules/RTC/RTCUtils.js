    ...
    if (um.indexOf("video") >= 0) {
        ...
        if (browser.isWebKitBased()) {
            if (constraints.video.height && constraints.video.height.ideal) {
                ...
                logger.warn(
                    "Ideal camera height missing, camera may not start properly",
                );
            }
            if (constraints.video.width && constraints.video.width.ideal) {
                ...
                logger.warn(
                    "Ideal camera width missing, camera may not start properly",
                );
            }
        }
...
function compareAvailableMediaDevices(newDevices) {
    ...
    return (
        ...
        availableDevices.map(mediaDeviceInfoToJSON).sort().join("")
    );
...
function sendDeviceListToAnalytics(deviceList) {
    const audioInputDeviceCount = deviceList.filter(
        (d) => d.kind === "audioinput",
    ).length;
    const audioOutputDeviceCount = deviceList.filter(
        (d) => d.kind === "audiooutput",
    ).length;
    const videoInputDeviceCount = deviceList.filter(
        (d) => d.kind === "videoinput",
    ).length;
    const videoOutputDeviceCount = deviceList.filter(
        (d) => d.kind === "videooutput",
    ).length;
...
class RTCUtils extends Listenable {
    ...
    _getUserMedia(umDevices, constraints = {}, timeout = 0) {
        return new Promise((resolve, reject) => {
            ...
            navigator.mediaDevices
                ...
                .catch((error) => {
                    logger.warn(
                        `Failed to get access to local media. ${error} ${JSON.stringify(constraints)}`,
                    );
    ...
    obtainAudioAndVideoPermissions(options) {
        ...
        const maybeRequestDesktopDevice = function () {
            ...
            const { desktopSharingSourceDevice, desktopSharingSources } =
                otherOptions;
    ...
    isUserStreamById(streamId) {
        return (
            streamId && streamId !== "mixedmslabel" && streamId !== "default"
        );
    }
}
...
function wrapAttachMediaStream(origAttachMediaStream) {
    return function (element, stream) {
        ...
        if (
            ...
            return element
                .setSinkId(rtcUtils.getAudioOutputDevice())
                .catch((ex) => {
