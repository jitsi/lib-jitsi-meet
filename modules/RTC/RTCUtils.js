...
function getConstraints(um = [], options = {}) {
        ...
        if (options.resolution) {
            if (Resolutions[options.resolution]) {
                constraints.video.width = {
                    ideal: Resolutions[options.resolution].width
                };
                constraints.video.height = {
                    ideal: Resolutions[options.resolution].height
                };
            }
        }
    } else {
    ...
    if (um.indexOf('audio') >= 0) {
        ...
        // Use the googXXX constraints on all chromium based browsers.
        } else {
...
class RTCUtils extends Listenable {
    ...
    init(options = {}) {
        ...
        screenObtainer.init(options, this.getUserMediaForScreensharing.bind(this));
    ...
    * on failure.
    **/
    getUserMediaForScreensharing(um, options = {}) {
        const constraints = getConstraints(um, options);

        logger.info('Get media constraints', constraints);

        return this._getUserMediaWithConstraints(um, constraints);
    }
     ...
     * @returns {Promise}
     */
    _getUserMediaWithConstraints(umDevices, constraints = {}) {
     ...
     * in RTCUtils#getUserMediaForScreensharing.
     *
     * @param {Object} options
    ...
    obtainAudioAndVideoPermissions(options = {}) {
        ...
        const maybeRequestDesktopDevice = function() {
            ...
            if (desktopSharingSourceDevice) {
                ...
                return this._getUserMediaWithConstraints(requestedDevices, constraints)
                    .then(stream => {
        ...
        const maybeRequestCaptureDevices = function() {
            ...
            const constraints = getConstraints(requestedCaptureDevices, options);

            logger.info('Got media constraints: ', constraints);

            return this._getUserMediaWithConstraints(requestedCaptureDevices, constraints);
        }.bind(this);
        ...
        const maybeCreateAndAddAVTracks = function(avStream) {
            ...
            if (audioTracks.length) {
                ...
                // Use the default constructor for MediaStream and then add the tracks since
                // the MediaStream constructed using mock MediaStreamTracks that we use in the
                // unit tests doesn't seem to return the audio and video mock tracks properly.
