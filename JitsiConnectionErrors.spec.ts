...
describe("/JitsiConnectionErrors members", () => {
    ...
    it("known members", () => {
        expect(CONFERENCE_REQUEST_FAILED).toBe(
            "connection.conferenceRequestFailed",
        );
        ...
        expect(JitsiConnectionErrors.CONFERENCE_REQUEST_FAILED).toBe(
            "connection.conferenceRequestFailed",
        );
        expect(JitsiConnectionErrors.CONNECTION_DROPPED_ERROR).toBe(
            "connection.droppedError",
        );
        expect(JitsiConnectionErrors.NOT_LIVE_ERROR).toBe(
            "connection.notLiveError",
        );
        expect(JitsiConnectionErrors.OTHER_ERROR).toBe("connection.otherError");
        expect(JitsiConnectionErrors.PASSWORD_REQUIRED).toBe(
            "connection.passwordRequired",
        );
        expect(JitsiConnectionErrors.SERVER_ERROR).toBe(
