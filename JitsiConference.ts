...
.toLowerCase();
   ...
   */
    ...
    /**
     * Triggers a client-initiated in-place ICE restart of the JVB session. The client rotates its own local ICE
     * credentials (as the offerer) and signals them to Jicofo, which relays them to the bridge; the bridge applies
     * them to its existing ice4j Agent. Media keeps flowing throughout (make-before-break). See
     * {@link JingleSessionPC.restartIce}. Trigger from the console: `APP.conference._room.restartJvbIce()`.
     *
     * @param {string} reason - Why the restart was triggered, for logs and analytics ('api', 'ice-failed', ...).
     * @returns {Promise<void>} - Resolves when the restart's renegotiation completes and the new local transport
     * has been signalled to Jicofo, rejects otherwise.
     */
    public restartJvbIce(reason: string = 'api'): Promise<void> {
        ...
        logger.info(`Requesting an in-place ICE restart of the JVB session, reason=${reason}`);
        RTCStats.sendStatsEntry(RTCStatsEvents.ICE_RESTART_REQUESTED_EVENT, null, { reason });
        Statistics.sendAnalytics(createJingleEvent(
            AnalyticsEvents.ACTION_JINGLE_ICE_RESTART_REQUESTED, { reason }));
