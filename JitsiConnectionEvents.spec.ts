...
describe("/JitsiConnectionEvents members", () => {
    ...
    it("known members", () => {
        expect(CONNECTION_DISCONNECTED).toBe(
        ...
        expect(JitsiConnectionEvents.CONNECTION_DISCONNECTED).toBe(
            "connection.connectionDisconnected",
        );
        expect(JitsiConnectionEvents.CONNECTION_ESTABLISHED).toBe(
            "connection.connectionEstablished",
        );
        expect(JitsiConnectionEvents.CONNECTION_FAILED).toBe(
            "connection.connectionFailed",
        );
        expect(JitsiConnectionEvents.CONNECTION_REDIRECTED).toBe(
            "connection.redirected",
        );
        expect(JitsiConnectionEvents.WRONG_STATE).toBe("connection.wrongState");
        expect(JitsiConnectionEvents.DISPLAY_NAME_REQUIRED).toBe(
            "connection.display_name_required",
        );
        expect(JitsiConnectionEvents.PROPERTIES_UPDATED).toBe(
            "connection.propertiesUpdated",
        );
    });
